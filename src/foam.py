    ...
    for i in range(len(model)):
        ...
        for y in range(partsize_y):
            for x in range(partsize_x):
                map[model[i][1] + y -
                    min_y][model[i][0] + x -
                           min_x] = part[(partsize_y - 1 - y) * partsize_x + x]
...
def plot_stacked_images(arrays,
                        figsize=(18, 3),
                        cmap='bwr',
                        colorbar=True,
                        titles=None):
    ...
    if titles is not None and len(titles) != num_images:
        raise ValueError(
            "Length of titles list must match the number of images.")
...
def scale_model(model, scale):
    ...
    return [(i * scale, ii * scale, iii * scale, iv * scale)
            for (i, ii, iii, iv) in model]
...
def get_foam(iteration=0):
    # Path to the OpenFOAM results
    data_path = Path(__file__).parents[1] / 'resources' / 'foam' / 'pitzDaily'
    if iteration == 0:
    ...
    model = [(0, 0, 1, 1), (1, 1, 10, 2), (1, 0, 10, 1)
            ]  # discribes, how the model is build, every block is a tupel
    scale = 10
...
if __name__ == "__main__":
    ux_map, uy_map, p_map = get_foam()
    u_map = np.sqrt(ux_map**2 + uy_map**2)
    # Plot results
    plot_stacked_images(
        [ux_map, uy_map, u_map, p_map],
        figsize=(9, 2),
        cmap='bwr',
        titles=['velocity X', 'velocity Y', 'velocity magnitude',
                'pressure'])  # plasma / bwr
