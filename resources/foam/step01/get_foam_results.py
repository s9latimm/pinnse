...
def scale_model(model, scale):
    ...
    return [(int(i * scale), int(ii * scale), int(iii * scale), int(iv * scale)) for (i, ii, iii, iv) in model]
...
def get_inlet_outlet(ux_map, uy_map):
    inlet_x = 0
    inlet_y = 0
    outlet_x = 0
    outlet_y = 0
    for i in range(len(ux_map)):
        inlet_x += ux_map[i][0]
        inlet_y += uy_map[i][0]
        outlet_x += ux_map[i][len(ux_map[0]) - 1]
        outlet_y += uy_map[i][len(uy_map[0]) - 1]
    print("inlet x: " + str(inlet_x))
    print("inlet y: " + str(inlet_y))
    print("outlet x: " + str(outlet_x))
    print("outlet y: " + str(outlet_y))
...
def get_normalized_maps(iteration = 0):
    ...
    #model = [(0,1.5,1,2),(0,0.5,1,1.5),(0,0,1,0.5),(1,1.5,2,2),(1,0,2,0.5),(2,1.5,10,2),(2,0.5,10,1.5),(2,0,10,0.5)] # discribes, how the model is build, every block is a tupel
    scale = 10
...
if __name__ == "__main__":
    ...
    get_inlet_outlet(ux_map, uy_map)
    # Plot results
